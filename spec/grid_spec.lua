...
describe("Grid", function ()
    ...
    describe("Resize", function ()
        it("bigger size", function ()
            local gr = grid.Grid(2, 2)
            local res = gr:resize(3, 3)
            assert.is.equal(res, true)
            assert.is.equal(gr.size_x, 3)
            assert.is.equal(gr.size_y, 3)
        end)

        it("lesser size", function ()
            local gr = grid.Grid(5, 5)
            local res = gr:resize(2, 2)
            assert.is.True(res)
            assert.is.equal(gr.size_x, 2)
            assert.is.equal(gr.size_y, 2)
        end)

        it("invalid size", function ()
            local gr = grid.Grid(5, 5)
            local res = gr:resize("a", "b")
            assert.is.False(res)
        end)
    end)
    ...
    describe("Get row", function ()
        ...
        it("row index less then grid size", function ()
            local gr = grid.Grid(4, 3, "A")
            local res = gr:get_row(0)
            assert.is.Table(res)
            assert.are.same(res, {})
        end)

        it("row index more then grid size", function ()
            local gr = grid.Grid(4, 6, "A")
            local res = gr:get_row(5)
            assert.is.Table(res)
            assert.are.same(res, {})
        end)
    end)
    ...
    describe("Traverse", function ()
        it("valid vector", function ()
            local gr = grid.Grid(2, 2, "B")
            local res = gr:traverse(1, 1, GRID_BOTTOM_LEFT)
            assert.is.Table(res)
            assert.are.same(res, {{1, 1, "B"}, {2, 2, "B"}})
        end)

        it("invalid base cell", function ()
            local gr = grid.Grid(2, 2, "B")
            local res = gr:traverse(0, 0, GRID_BOTTOM_LEFT)
            assert.is.Nil(res)
        end)
    end)
end)
